...
const endpoint = 'http://localhost:4000/allTasks'
var tasks = []
...
function findMatches(wordToMatch, tasks){
  ...
  //THE BELOW .data WAS NOT DONE IN VIDEO
...
const taskList = document.querySelector('.suggestions');
...
function searchTask(){
  searchBar.addEventListener('keyup', (i) => {
      ...
      const filteredTasks = tasksAll.filter(task => {
          return (
            task.taskName.toLowerCase().includes(searchString)
          );
      })
...
loadTasks();
  ...
  //let taskURL = "http://localhost:4000/allTasks" + taskIdParam;
  //from below, gives address just change to name
*/
